...
- (void)startRecording {
    ...
    self.timerTimeout = [NSTimer scheduledTimerWithTimeInterval:180 target:self selector:@selector(timeoutCheck:) userInfo:nil repeats:NO];
}
...
- (void)encapsulatingOver {
    if (self.delegate) {
        ...
        NSLog(@"录音结束,录音时长：%.0f",[self recordedTimeInterval]);
    }
}

- (NSTimeInterval)recordedTimeInterval {
    
    NSLog(@"开始：%.0f  结束:%.0f",[self.dateStartRecording timeIntervalSinceReferenceDate],[self.dateStopRecording timeIntervalSinceReferenceDate]);
    return (self.dateStopRecording && self.dateStartRecording) ? [self.dateStopRecording timeIntervalSinceDate:self.dateStartRecording] : 0;
}
